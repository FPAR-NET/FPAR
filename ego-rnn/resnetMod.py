...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, noBN=False):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.weight.data.normal_(0, math.sqrt(2. / n)) # normal init of weights (Xavier) (original was nn.init.kaiming_normal_)
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)   #flatten in the original (is it the same I suppose)
        x = self.fc(x)
        if self.noBN:
            return x, conv_layer4BN, conv_layer4NBN
        else:
            return x, conv_layer4BN     #both output prediction and layer4 weights are returned! the weights are needed for CAM!
